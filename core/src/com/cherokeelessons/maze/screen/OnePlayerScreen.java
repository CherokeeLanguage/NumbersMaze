...
public class OnePlayerScreen extends ScreenBase {
	...
	final private String startupMessage = ""
			+ " * Explode dice in combinations to equal the challenge number.\n"
	...
	private Group betweenOptions=new Group();
	...
	public OnePlayerScreen(NumbersMaze _app, DataBundle _data) {
		super();
		app=_app;
		data.put(_data.get());
	...
	private void createLevelCompleteMsgBox(){
		menuItems.clear();
		String lvlUpMessage="Level Completed: "+(data.getInteger("level")-1)+"\n" +
		...
		msgBox.pack();
		float w = msgBox.getWidth();
		float h = msgBox.getHeight();
